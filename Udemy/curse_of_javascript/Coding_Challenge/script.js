...
const calcAverageHumanAge = function (ages) {
  const humanAges = ages.map(age =>
    age <= 2 ? (humanAge = age * 2) : (humanAge = 16 + age * 4)
  );
  console.log(
    `Calculating the age of the dog to be human is: ${humanAges.join(', ')}`
  );
  const adult = humanAges.filter(age => age > 18);
  console.log(`Dogs adult ${adult.join(', ')}`);

  const averageAdult = adult.reduce((acc, age) => acc + age) / adult.length;
...
1. Loop over the array containing dog objects, and for each dog, calculate the recommended food portion and add it to the object as a new property. Do
NOT create a new array, simply loop over the array. Forumla: recommendedFood = weight ** 0.75 * 28. (The result is in grams of food, and the weight needs
to be in kg).
